{
  ...
  pfAnchor = "com.apple/nix-chips/traefik";
  pfAnchorFile = pkgs.writeText "traefik-pf.anchor" ''
    # Generated by nix-chips loopback-setup
    rdr pass on lo0 inet proto tcp from any to ${cfg.address} port 80 -> ${cfg.address} port ${toString traefikHttpPort}
    rdr pass on lo0 inet proto tcp from any to ${cfg.address} port 443 -> ${cfg.address} port ${toString traefikHttpsPort}
  '';
  ...
  loopback-setup = pkgs.writeShellScriptBin "loopback-setup" (
    ...
    then ''
      ...
      # Redirect port 80 to Traefik's unprivileged port via PF.
      if ! sudo pfctl -s info 2>/dev/null | grep -q "Status: Enabled"; then
        echo "Enabling PF firewall..."
        sudo pfctl -E
      fi

      echo "Loading PF redirect rules from ${pfAnchorFile} into ${pfAnchor}..."
      sudo pfctl -a "${pfAnchor}" -f "${pfAnchorFile}"
    ''
    else ''
      set -euo pipefail
...
in {
  config = lib.mkMerge [
    ...
    (lib.mkIf isDarwin {
      ports.http = lib.mkDefault traefikHttpPort;
      ports.https = lib.mkDefault traefikHttpsPort;
    })
    (lib.mkIf needsSetup {
      devShell = {
