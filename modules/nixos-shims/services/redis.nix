...
in {
  imports = [(modulesPath + "/services/databases/redis.nix")];

  config = mkIf (enabledServers != {}) {
    systemd.services =
      mapAttrs' (
        name: conf: let
          newSettings = removeAttrs conf.settings [
            "dir"
            "unixsocket"
            "unixsocketperm"
          ];
        in
          nameValuePair (redisName name) {
            serviceConfig = {
              ExecStart = mkForce "${cfg.package}/bin/${
                cfg.package.serverBin or "redis-server"
              } ${redisConfig newSettings} ${escapeShellArgs conf.extraParams}";
