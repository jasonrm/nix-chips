{
  ...
  safeGetReadOnly = opt: let
    tried = builtins.tryEval (opt.readOnly or false);
  in
    if tried.success
    then tried.value
    else false;
  ...
  walkOptions = depth: prefix: tree:
    ...
    then {}
      ...
      names =
        if namesTried.success
        then namesTried.value
        else [];
    in
      builtins.foldl' (
        acc: name: let
        ...
        in
          ...
          in
            ...
            in
              if isAttrsTried.success && isAttrsTried.value
              then let
                subTried = builtins.tryEval (walkOptions (depth + 1) fullName val);
              in
                if subTried.success
                then acc // subTried.value
                else acc
              else acc
      ) {}
      names;
  ...
  relevantOpts = lib.filterAttrs (n: _:
    builtins.elem n [
      "arcanum"
      "devShell"
      "dir"
