...
pkgs.writeShellScriptBin "arcanum" ''
  ...
  RECIPIENTS=$(${pkgs.nix}/bin/nix --extra-experimental-features "nix-command flakes" eval --json '.#lib.arcanum' | ${pkgs.jq}/bin/jq -r "if .\"$ENCRYPTED_FILE\" then .\"$ENCRYPTED_FILE\" else [] end | join(\"\n\")")
